...
#   MCP_OAUTH_AUTHORIZATION_SERVER_METADATA_ISSUER (optional HTTPS issuer for local metadata compatibility shim)
#   MCP_OAUTH_AUTHORIZATION_ENDPOINT    (required with metadata issuer)
#   MCP_OAUTH_TOKEN_ENDPOINT            (required with metadata issuer)
...
MCP_LOG_LEVEL="${MCP_LOG_LEVEL:-WARNING}"
...
MCP_OAUTH_AUTHORIZATION_SERVER_METADATA_ISSUER="${MCP_OAUTH_AUTHORIZATION_SERVER_METADATA_ISSUER:-}"
...
if [[ -n "${MCP_OAUTH_AUTHORIZATION_SERVER_METADATA_ISSUER}" ]] && { [[ -z "${MCP_OAUTH_AUTHORIZATION_ENDPOINT}" ]] || [[ -z "${MCP_OAUTH_TOKEN_ENDPOINT}" ]]; }; then
  echo "ERROR: MCP_OAUTH_AUTHORIZATION_ENDPOINT and MCP_OAUTH_TOKEN_ENDPOINT are required when MCP_OAUTH_AUTHORIZATION_SERVER_METADATA_ISSUER is set." >&2
  exit 2
fi
