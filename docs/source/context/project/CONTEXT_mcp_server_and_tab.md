...
This context file documents how the MCP server (`mcp_server.py`) and the Embedded MCP Tab (`mcp_tab.py`) enable external Model Context Protocol (MCP) clients to directly query OCI Identity and Policies and expose focused search/filtering tools.
...
- **MCP Server (`mcp_server.py`)**
  - Launches a [FastMCP](https://gofastmcp.com/getting-started/welcome) server exposing tools and resources for policy and identity queries.
  ...
  - Uses the shared application service layer for load and query operations so MCP remains a thin protocol adapter over the same model used by the web and desktop consumers.
...
- **Embedded MCP Tab (`mcp_tab.py`)**
  ...
  - When launching MCP, passes in the application's active service context so the server exposes the same loaded in-memory state through service facades.

---

## 2. Tool Exposure and Data Sources

**Tools exposed by the MCP server are grouped by service responsibility:**
...
- **Load Service**
  - Standalone MCP loading uses the shared cache and tenancy loading flow.
  - Reloads use the same service path instead of duplicating OCI load/cache behavior in the MCP adapter.
...
- `mcp.md` covers:
    ...
    - Tips and scenarios for focused policy and identity search from desktop, proxy, or remote deployments
...
**Summary:**  
The MCP server architecture (standalone or embedded via the MCP tab) surfaces fine-grained tools for policy and identity analysis. Its modular design allows tools to be sourced from the core data set or advanced overlays, and places focused search capabilities at the fingertips of any MCP-compliant client. Refer to `mcp.md` for all specific tool semantics and extension guidelines.
