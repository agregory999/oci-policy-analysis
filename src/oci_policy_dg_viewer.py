...
def update_policy_output():
    ...
    rows_to_show = [
        ...
        if (
            chk_show_service.get()
            and st[6] == 'service'
            or chk_show_dynamic.get()
            and st[6] == 'dynamic-group'
            or chk_show_resource.get()
            and st[6] == 'resource'
            or chk_show_regular.get()
            and st[6] in ['group', 'any-user', 'any-group']
            or chk_show_invalid.get()
            and not st[5]
        )
    ]
    ...
    if rows_to_show and len(rows_to_show) == 0:
        ...
        sheet_policies.display_rows(rows=rows_to_show, all_rows_displayed=False)
...
def format_dgrule(text, indent_level=0):  # noqa: C901
    """
...
def update_cross_tenancy_output():
    ...
    sheet_cross_tenancy_policies.display_columns(columns=[0, 1, 2, 3, 4], all_columns_displayed=False)
...
def update_selection_info():
    ...
    # compartment_id = compartment_var.get()
    domain_display = (
        next((d['display_name'] for d in identity_domain_analysis.get_domains() if d['id'] == domain_id), 'None')
        if domain_id != 'None'
        else 'None'
    )
    user_display = (
        next(
            (u['display_name'] for u in identity_domain_analysis.get_users_by_domain(domain_id) if u['id'] == user_id),
            'None',
        )
        if user_id != 'None'
        else 'None'
...
def sort_sheet_cross_tenancy_define(col):
    ...
    sheet_cross_tenancy_define.sort_rows_by_column(
        column=seleced_column_index, reverse=sheet_cross_tenancy_define_search_order
    )
...
def sort_sheet_cross_tenancy_policies(col):
    ...
    seleced_column_index = col['selected'].column
    sheet_cross_tenancy_policies.sort_rows_by_column(
        column=seleced_column_index, reverse=sheet_cross_tenancy_policies_search_order
    )
...
def export_user_to_csv():
    ...
    if filepath:
        with open(filepath, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            writer.writerows(sheet_user_policies.data)
        logger.info(f'Exported {len(sheet_user_policies.data)} user policy statements to {filepath}')
...
def run_dg_analysis():
    logger.info(
        f'Running Dynamic Group Analysis for {len(identity_domain_analysis.dynamic_groups)} DGs and {len(policy_compartment_analysis.regular_statements)} Policies'
    )
...
def update_user_dropdown():
    ...
    if domain_id != 'None':
        users = [
            u['display_name']
            for u in sorted(identity_domain_analysis.get_users_by_domain(domain_id), key=lambda x: x['display_name'])
        ]
        if not users:
            users = ['None']
    ...
    for user in users:
        user_id = (
            next(
                (u['id'] for u in identity_domain_analysis.get_users_by_domain(domain_id) if u['display_name'] == user),
                'None',
            )
            if user != 'None'
            else 'None'
...
def update_domain_dropdown():
    domains = ['None'] + [
        d['display_name'] for d in sorted(identity_domain_analysis.get_domains(), key=lambda x: x['display_name'])
    ]
    ...
    for domain in domains:
        domain_id = (
            next((d['id'] for d in identity_domain_analysis.get_domains() if d['display_name'] == domain), 'None')
            if domain != 'None'
            else 'None'
        )
        domain_dropdown['menu'].add_command(
            label=domain,
            command=lambda d=domain, did=domain_id: [
                domain_var.set(did),
                domain_display_var.set(d),
...
def update_principals_sheets(*args):
    ...
    principals_sheet_dynamic_groups.grid_forget()  # Hide dynamic group sheet
    principals_sheet_policies_instance.grid_forget()  # Hide policy sheet temporarily
    ...
    if principals_style == 'any-user':
        ...
        if resource_type == 'Any':
            ...
            policies = policy_compartment_analysis.filter_policy_statements(condition_filter='request.principal.type')
            ...
            policies = policy_compartment_analysis.filter_policy_statements(condition_filter=condition_filter)
            principals_sheet_policies_instance.set_sheet_data(policies)

        # UI Elements - re-grid with only Policy viewer
        principals_sheet_policies_instance.grid(row=0, column=0, rowspan=2, sticky='nsew')  # Show policy sheet in row 1
        ...
        filtered_dynamic_groups = identity_domain_analysis.filter_dynamic_groups(
        ...
        policies = policy_compartment_analysis.filter_policy_statements(subj_filter=subject_search)
...
def main():
    ...
    global \
        ...
        dg_btn_export, \
        use_subject_any, \
        location_filter_tenancy, \
        hierarchy_filter_root, \
        policy_compartment_analysis, \
        identity_domain_analysis, \
        last_load_time, \
        label_profile, \
        btn_load_tenancy, \
        btn_load_cache, \
        principals_sheet_dynamic_groups, \
        principals_sheet_policies_instance, \
        tab_users, \
        domain_var, \
        domain_display_var, \
        user_var, \
    ...
    # Policy Tab
    global chk_show_service, chk_show_dynamic, chk_show_resource, chk_show_regular, chk_show_invalid, chk_show_expanded

    # Dynamic Group Tab
    global chk_show_instance_principals, dg_entry_domain, dg_entry_name, dg_entry_type, dg_entry_ocid

    # User Report Tab
    global text_dg_report, text_policy_report, highlight_entry_var, text_user_details, btn_export_user

    # Resource Principal Tab
    global principals_style_var, resource_type_var, resource_type_dropdown, principals_style_dropdown

    # Cross-tenancy tab
    global \
        sheet_cross_tenancy_policies, \
        sheet_cross_tenancy_define, \
        sheet_cross_tenancy_define_search_order, \
        sheet_cross_tenancy_policies_search_order
    ...
    ttk.Checkbutton(
        frm_init, text='Instance Principal', variable=use_instance_principal, command=toggle_profile_dropdown
    ...
    ttk.Label(frm_policy_filter, text='Filters (| for OR, AND between fields)').grid(
    ...
    ttk.Checkbutton(frm_subj, text='Any-User/Group', variable=use_subject_any, command=toggle_any_subject).grid(
    ...
    ttk.Checkbutton(frm_loc, text='Tenancy', variable=location_filter_tenancy, command=toggle_location_tenancy).grid(
        row=0, column=1, padx=2
    )
    ...
    ttk.Checkbutton(frm_hierarchy, text='Root', variable=hierarchy_filter_root, command=toggle_hierarchy_root).grid(
        row=0, column=1, padx=2
    )
    ...
    btn_export_policy = ttk.Button(
        frm_policy_buttons, text='Export Filtered\nStatements to CSV', state=tk.DISABLED, command=export_policy_to_csv
    )
    ...
    ttk.Checkbutton(frm_policy_output, text='Service', variable=chk_show_service, command=update_policy_output).grid(
        row=0, column=3, padx=5, pady=3
    )
    ttk.Checkbutton(
        frm_policy_output, text='Dynamic Group', variable=chk_show_dynamic, command=update_policy_output
    ).grid(row=0, column=4, padx=5, pady=3)
    ttk.Checkbutton(frm_policy_output, text='Resource', variable=chk_show_resource, command=update_policy_output).grid(
        row=0, column=5, padx=5, pady=3
    )
    ttk.Checkbutton(frm_policy_output, text='Regular', variable=chk_show_regular, command=update_policy_output).grid(
        row=0, column=6, padx=5, pady=3
    )
    ttk.Checkbutton(frm_policy_output, text='Invalid', variable=chk_show_invalid, command=update_policy_output).grid(
        row=0, column=7, padx=5, pady=3
    )
    ttk.Checkbutton(
        frm_policy_output, text='Expanded Output', variable=chk_show_expanded, command=update_policy_output
    ...
    dg_btn_export = ttk.Button(
        frm_dg_buttons, text='Export Selected\nDGs to CSV', state=tk.DISABLED, command=export_dg_to_csv
    )
    ...
    ttk.Checkbutton(
        ...
        variable=chk_show_instance_principals,
        command=update_dg_output,
    ).grid(row=0, column=2, padx=5, pady=3)
    ttk.Checkbutton(
        frm_dg_output, text='Highlight Unused Dynamic Groups', variable=chk_show_not_in_use, command=update_dg_output
    ...
    principals_style_list = ['Dynamic Group', 'any-user']
    principals_style_dropdown = ttk.OptionMenu(
        frm_top, principals_style_var, principals_style_var.get(), *principals_style_list
    )
    ...
    resource_type_dropdown = ttk.OptionMenu(frm_top, resource_type_var, resource_type_var.get(), *resource_type_list)
    ...
    frm_bottom.grid_rowconfigure(1, weight=1)
    ...
    principals_sheet_dynamic_groups.enable_bindings(
    ...
    principals_sheet_policies_instance.grid(row=1, column=0, sticky='nsew')
    principals_sheet_policies_instance.enable_bindings('column_width_resize', 'row_select', 'copy')
    principals_sheet_policies_instance.display_columns(
    ...
    user_dropdown = ttk.OptionMenu(
        frm_user_select, user_display_var, 'None', 'None', command=lambda _: update_user_analysis_output()
    )
    ...
    btn_export_user = ttk.Button(
        frm_user_buttons, text='Export User Policies to CSV', state=tk.DISABLED, command=export_user_to_csv
    )
    ...
    ttk.Checkbutton(
        frm_user_output, text='Expanded', variable=chk_show_expanded, command=update_user_analysis_output
    ...
    btn_export_report = ttk.Button(
        frm_report_buttons, text='Export Report', state=tk.DISABLED, command=export_report_to_txt
    )
