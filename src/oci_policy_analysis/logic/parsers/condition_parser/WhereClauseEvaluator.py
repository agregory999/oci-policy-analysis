    ...
    def visitSingle_condition(self, ctx):
        ...
        try:
            if ctx.literal_list():
                ...
                logger.debug("literal_list_content children type/value: %s", [(type(c).__name__, c.getText()) for c in list_content.getChildren()])
                ...
                value_token_type = (
                    'PATTERN_LITERAL' if is_pattern
                    else 'STRING_LITERAL' if is_string
                    else 'IDENTIFIER' if is_identifier
                    else 'UNKNOWN'
                )
                if is_pattern:
                    value = value_raw
        ...
        if operator == 'between':
            if variable != 'request.utc-timestamp.time-of-day':
                logger.warning(f"Operator 'between' is only valid with 'request.utc-timestamp.time-of-day' (got '{variable}')")
        ...
        try:
            if operator == '=':
                if value_token_type == 'PATTERN_LITERAL' and isinstance(value, str) and value.startswith('/') and value.endswith('/'):
                    ...
                    compare_target_stripped = (compare_target.strip() if isinstance(compare_target, str) else compare_target)
                    try:
                ...
                logger.info(f"'in' operator debug: variable={variable!r}, sim_value={sim_value!r}, value_list={value!r}")
                ...
                if isinstance(value, list):
                    ...
                    for v in value:
                        ...
                        if cand_str.lower() == norm_v.lower():
                            logger.info(f"IN MATCH: candidate '{cand_str}' (case-insensitive) matched list item '{norm_v}'")
                            comparison_result = True
                            log_entry['sim_value'] = cand_str
                            log_entry['expected'] = str(value)
                            log_entry['operator'] = operator
                            log_entry['type'] = f"List case-insensitive 'in'"
                            log_entry['result'] = True
                            self.comparison_log.append(log_entry.copy())
                            match_found = True
                            break
                    if not match_found:
                        logger.warning(f"IN NO MATCH: candidate '{cand_str}' did not match any (case-insensitive) in {value!r}")
                        comparison_result = False
                        log_entry['sim_value'] = cand_str
                        log_entry['expected'] = str(value)
                        log_entry['operator'] = operator
                    ...
                    norm_v = value.strip().strip('"').strip("'") if isinstance(value, str) else (value if value is not None else "")
                    candidate = sim_value or ''
                    cand_str = str(candidate).strip()
                    ...
                    if result:
                        ...
                        log_entry['operator'] = operator
                        log_entry['type'] = f"Scalar case-insensitive 'in'"
                        log_entry['result'] = True
                        self.comparison_log.append(log_entry.copy())
                    else:
