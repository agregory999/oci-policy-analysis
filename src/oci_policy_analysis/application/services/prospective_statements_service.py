...
# Backward-compatible settings key retained for tests and legacy callers.
SETTINGS_KEY_PROSPECTIVE_BY_TENANCY = 'simulation_prospective_statements_by_tenancy'


class _SettingsProspectsAdapter:
    """Adapter exposing load/save_prospects over legacy settings dict storage."""
    ...
    def load_prospects(self, tenancy_ocid: str) -> list[dict[str, Any]]:
        by_tenancy = self._settings.get(SETTINGS_KEY_PROSPECTIVE_BY_TENANCY, {})
        if not isinstance(by_tenancy, dict):
            return []
        rows = by_tenancy.get(tenancy_ocid, [])
    ...
    def save_prospects(self, tenancy_ocid: str, rows: list[dict[str, Any]]) -> None:
        ...
        if not isinstance(by_tenancy, dict):
            by_tenancy = {}
            self._settings[SETTINGS_KEY_PROSPECTIVE_BY_TENANCY] = by_tenancy
        by_tenancy[tenancy_ocid] = list(rows)
...
class ProspectiveStatementsService:
    ...
    def __init__(
        ...
        *,
        ...
        # Accept both modern cache-manager based flow and legacy settings dict flow.
        self._cache = cache_manager if cache_manager is not None else _SettingsProspectsAdapter(settings)
    ...
    def settings(self) -> dict[str, Any] | None:
        """Return underlying legacy settings dict when using adapter mode."""
        adapter = self._cache
        if isinstance(adapter, _SettingsProspectsAdapter):
            return adapter._settings
        return None
    ...
    def validate_and_update_text(
        ...
        # - when a simulation engine is provided and supports
        #   validate_prospective_statement, defer to it.
        # - when no simulation engine is available, keep the record as
        #   unvalidated (parsed=False/valid=False).
        if self._simulation_engine is None or not hasattr(self._simulation_engine, 'validate_prospective_statement'):
            rec.parsed = False
            rec.valid = False
            rec.invalid_reasons = [
                'Simulation engine is not available for prospective statement validation.',
            ]
            rec.normalized = None
            self._ensure_effective_path(rec)
            return rec
        ...
        try:
            result = self._simulation_engine.validate_prospective_statement(rec.statement_text)
    ...
    def persist_and_push_to_engine(self) -> None:
        ...
        * Writes the same list to settings under the
        ...
        if self._simulation_engine is not None and hasattr(self._simulation_engine, 'set_prospective_statements'):
            try:
                self._simulation_engine.set_prospective_statements(simple_list)
            except Exception:  # pragma: no cover - defensive
                logger.warning(
                    'ProspectiveStatementsService: unable to push prospective statements to simulation engine',
                    exc_info=True,
                )
