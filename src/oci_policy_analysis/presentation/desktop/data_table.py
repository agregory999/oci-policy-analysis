...
class DataTable(ttk.Frame):
    ...
    Note: ttk.Treeview does not natively support row-specific heights. This
    widget adjusts the whole table's row height to fit the largest visible
    explicit newline count in the current row set, capped to keep tables usable.
    ...
    def _setup_ui(self) -> None:
        ...
        self._style.configure(self._tree_style_name, padding=(0, 0, 8, 0))
        treeview_args = {
    ...
    def _populate_data(self) -> None:
        ...
        self._update_dynamic_row_height()
    ...
    def _update_dynamic_row_height(self) -> None:
        """Adjust this table's row height for explicit multiline cell values."""

        visible_columns = list(self.tree['columns']) if hasattr(self, 'tree') else list(self.display_columns)
        max_lines = 1
        for row in self.data:
            for col in visible_columns:
                ...
                if max_lines >= self.max_auto_row_lines:
                    break
            if max_lines >= self.max_auto_row_lines:
                break

        max_lines = max(1, min(max_lines, self.max_auto_row_lines))
        line_height = self._get_treeview_line_height()
        row_height = max(20, (line_height * max_lines) + 8)
        self._style.configure(self._tree_style_name, rowheight=row_height)
        logger.debug(
            'DataTable rowheight set to %d for %d visible line(s)',
            row_height,
            max_lines,
        )

    def _get_treeview_line_height(self) -> int:
        """Return the current line height for the table font."""

        font_spec = self._style.lookup(self._tree_style_name, 'font') or self._style.lookup('Treeview', 'font')
        try:
            if font_spec:
                return int(tkfont.Font(root=self, font=font_spec).metrics('linespace'))
            return int(tkfont.nametofont('TkDefaultFont').metrics('linespace'))
        except Exception:
            return 16
